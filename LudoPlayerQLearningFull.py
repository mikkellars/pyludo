...
class LudoPlayerQLearningFull:
    ...
    def policies(self, QTable, epsilon, state, next_states): # Inspiration from https://www.geeksforgeeks.org/q-learning-in-python/?fbclid=IwAR1UXR88IuJBhhTakjxNq_gcf3nCmJB0puuoA46J8mZnEan_qx9hhoFzhK8
        ...
        def epsilonGreedyPolicy(): 
            ...
            while not valid_actions[best_action]:
                best_action = np.argsort(QTable[tmp_state])[i]
                i -= 1
    ...
    def QLearning(self, state, next_states):  # Inspiration from https://www.geeksforgeeks.org/q-learning-in-python/?fbclid=IwAR1UXR88IuJBhhTakjxNq_gcf3nCmJB0puuoA46J8mZnEan_qx9hhoFzhK8
        ...
        self.__updateQTable(state.state[0], np.array([0.0, 0.0, 0.0, 0.0]))
        ...
        action = np.random.choice( LudoPlayerQLearningFull.actions, p=actionProbability )

        # Find next state based on action and updates Q-table. 
        next_states_based_action = str(next_states[action].state[0]) 
        ...
        self.__updateQTable(next_states_based_action, np.array([0.0, 0.0, 0.0, 0.0]))

        # Update based on TD Update
        td_target = reward + self.__discount_factor * np.max(self.__QTable[str(next_states_based_action)])
        td_delta = td_target - self.__QTable[str(state.state[0])][action] 
        update_val = self.__alpha * td_delta 

        self.__QTable[str(state.state[0])][action] += update_val
