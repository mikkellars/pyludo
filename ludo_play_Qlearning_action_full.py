...
from LudoPlayerQLearningFullAction import LudoPlayerQLearningFullAction
...
def normal_training():
    ...
    players = [LudoPlayerRandom() for _ in range(3)]
    players.append(LudoPlayerQLearningFullAction("epsilon greedy", QtableName='1_QTable_action_full_r_cum', RewardName='1_Reward_action_full_r_cum', epsilon=0.1, discount_factor=0.5, learning_rate=0.1))
    for i, player in enumerate(players):
        player.id = i # selv tildele atributter uden defineret i klassen
    ...
    Plot = PlotStatistics()
    Plot.plotReward(pathToCSV='1_Reward_action_full_r_cum_e-0.1_d-0.5_a-0.1.csv', numMovAvg=1000)
