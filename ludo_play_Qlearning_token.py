...
def normal_training():
    ...
    players = [LudoPlayerRandom() for _ in range(3)]
    players.append(LudoPlayerQLearning("epsilon greedy", QtableName='QTable_token', RewardName='Reward_token', epsilon=0.1, discount_factor=0.5, learning_rate=0.1))
    for i, player in enumerate(players):
        player.id = i # selv tildele atributter uden defineret i klassen
    ...
    Plot = PlotStatistics()
    Plot.plotReward(pathToCSV='Reward_token_e-0.1_d-0.5_a-0.1.csv', numMovAvg=1000)
