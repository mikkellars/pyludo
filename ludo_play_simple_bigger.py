...
from LudoPlayerQLearningSimpleBigger import LudoPlayerQLearningSimpleBigger


def main():
    players = []
    players = [LudoPlayerRandom() for _ in range(3)]
    t1 = LudoPlayerQLearningSimpleBigger("epsilon greedy", QtableName='1_QTable_simple_bigger', RewardName='1_Reward_simple_bigger', epsilon=0.1, discount_factor=0.5, learning_rate=0.1)
    players.append(t1)
    ...
    for i in tqdm_1:
        ...
        for player in players: # Saving reward for QLearning player
            if type(player)==LudoPlayerQLearningSimpleBigger:
    ...
    Plot = PlotStatistics()
    Plot.plotReward(pathToCSV='1_Reward_simple_bigger_e-0.1_d-0.5_a-0.1.csv', numMovAvg=1000)
